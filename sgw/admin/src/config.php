...
/**
 * 当前订阅路径前缀（XBoard 默认 /s/，可在后台设置中覆盖）。
 */
function get_subscribe_path_prefix(): string {
    static $cached = null;
    if ($cached !== null) return $cached;

    global $_sg;
    if (!empty($_sg['subscribe_path'])) {
        $cached = $_sg['subscribe_path'];
        return $cached;
    }
    if (file_exists(PROTECT_CONF)) {
        $pc = @file_get_contents(PROTECT_CONF);
        if ($pc !== false && preg_match('/^location\s+\^~\s+(\S+)/m', $pc, $m)) {
            $cached = $m[1];
            return $cached;
        }
    }
    $cached = DEFAULT_SUBSCRIBE_PATH;
    return $cached;
}

/**
 * 从 nginx 日志 request 字段提取 XBoard 订阅 Token（/s/{token} 或 /s/{token}?...）。
 */
function extract_subscribe_token(string $request): string {
    $quoted = preg_quote(get_subscribe_path_prefix(), '#');
    if (preg_match('#' . $quoted . '([^/?\s"]+)#', $request, $m)) {
        return $m[1];
    }
    return '';
}
