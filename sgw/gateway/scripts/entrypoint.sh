...
write_admin_proxy_conf() {
    local output="/etc/nginx/subscribe/admin_proxy.conf"
    local secret="${ADMIN_SECRET_PATH:-}"

    if [[ -z "$secret" ]]; then
        echo "# Admin proxy disabled: ADMIN_SECRET_PATH not set" > "$output"
        log "管理后台 443 反代未启用：未配置 ADMIN_SECRET_PATH"
        return
    fi

    if [[ ! "$secret" =~ ^[a-zA-Z0-9_-]+$ ]]; then
        echo "# Admin proxy disabled: invalid ADMIN_SECRET_PATH" > "$output"
        log "[警告] ADMIN_SECRET_PATH 含非法字符，跳过管理后台 443 反代"
        return
    fi

    cat > "$output" <<EOF
# Generated by entrypoint.sh — admin UI via gateway :443 (CDN-friendly)
location ^~ /${secret} {
